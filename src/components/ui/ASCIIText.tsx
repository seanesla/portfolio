...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, opts: { fontSize?: number; fontFamily?: string; charset?: string; invert?: boolean } = {}) {
    ...
    ;(this.context as unknown as Record<string, boolean>).webkitImageSmoothingEnabled = false
...
class CanvAscii {
  ...
  resume() {
    if (this.animationFrameId === 0) this.animate()
  }
  ...
  clear() {
    this.scene.traverse((obj) => {
      if ((obj as THREE.Mesh).isMesh) {
        ...
        if (m.material && typeof m.material === 'object') {
          const mat = m.material as THREE.ShaderMaterial
          Object.keys(mat).forEach((key) => {
            const prop = (mat as unknown as Record<string, unknown>)[key]
            if (prop && typeof prop === 'object' && 'dispose' in prop && typeof (prop as { dispose: () => void }).dispose === 'function') {
      ...
      if (!cancelled && asciiRef.current) {
        ...
        visObserver = new IntersectionObserver(
          ([entry]) => {
            ...
            if (entry.isIntersecting) asciiRef.current.resume()
            else asciiRef.current.pause()
          },
          { threshold: 0 }
        )
    ...
    >
      <style>{`
        ...
        .ascii-text-container pre {
          ...
          left: 0;
          top: 0;
          background-image: radial-gradient(circle, rgba(140, 180, 255, 0.95) 0%, rgba(200, 220, 255, 0.8) 50%, rgba(255, 255, 255, 0.6) 100%);
