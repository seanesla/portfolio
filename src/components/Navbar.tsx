...
export default function Navbar({ visible }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const lifeST = lifeEl
        ? ScrollTrigger.getAll().find((t) => t.trigger === lifeEl)
        : undefined;

      const projectsEl = document.querySelector('[data-nav-id="projects"]');
      const projectsST = projectsEl
        ? ScrollTrigger.getAll().find((t) => t.trigger === projectsEl)
        : undefined;

      // Build cached triggers for non-life sections
      cachedTriggersRef.current = [];
      if (projectsST)
        cachedTriggersRef.current.push({ navId: "projects", st: projectsST });
      ...
      const onScroll = () => {
        ...
        requestAnimationFrame(() => {
          ...
          if (
            lifeST &&
            scrollY >= lifeST.start - vh * 0.3 &&
            scrollY <= lifeST.end + vh * 0.1
          ) {
          ...
          if (!active) {
            for (const { navId, st } of cachedTriggersRef.current) {
              if (
                scrollY >= st.start - vh * 0.3 &&
                scrollY <= st.end + vh * 0.1
              ) {
                active = navId;
              }
            }
  ...
  return (
    <div className="fixed top-4 inset-x-0 z-[100] flex justify-center pointer-events-none">
      ...
      >
        ...
        >
          <nav
            className="relative flex items-center gap-1 px-2 overflow-x-auto"
            style={
              {
                scrollbarWidth: "none",
          ...
          >
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 50% 40%, rgba(140,180,255,0.18), rgba(140,180,255,0.08))",
                border: "1px solid rgba(140,180,255,0.12)",
                boxShadow:
                  "0 0 12px rgba(140,180,255,0.06), inset 0 1px 0 rgba(255,255,255,0.05)",
                opacity: 0,
